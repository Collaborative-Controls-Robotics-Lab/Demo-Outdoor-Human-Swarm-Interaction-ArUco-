...
load camera_parameters_1280_720.mat -mat camMatrix distCoeffs
...
if simulate_flag
    ...
    robotPositionXYZ0 = -[robotPose.positionXYZ]; % Filpped due to vicon system calibration
    ...
    robotPose = viconPose(robotIndicesVicon);
    robotPositionXYZ = -[robotPose.positionXYZ]; % Filpped due to vicon system calibration
...
Goal1 =  [[-2.5,-2.5,-1.5,-1.5]+0.85;-1.7,-0.7,-0.7,-1.7];
...
hObstacle1 = patch(Obstacle1(1,:),Obstacle1(2,:),'FaceColor',[1 0 0],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.75);
hObstacle2 = patch(Obstacle2(1,:),Obstacle2(2,:),'FaceColor',[1 0 0],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.75);
...
hBlueBox = patch(Goal1(1,:),Goal1(2,:),'FaceColor',[0.25 0.5 1],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.2);
hGreenBox = patch(Goal2(1,:),Goal2(2,:),'FaceColor',[0 1 0.5],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.2);
...
for ii = 1:numRobots
    hVoronoi(:,ii) = patch(nan,nan,'FaceColor',colorMat(ii,:),'FaceAlpha',0.25);
end
...
for t = 1:iterations
    ...
    elseif algorithmPhase == 2 % Coverage control
        ...
        if collision_flag
            set(hSubdomPlot,'XData',subDom([1 1 2 2]),'YData',subDom([3 4 4 3]),'EdgeColor','r')
            % extDisp.set(hSubdomPlotProjector,'XData',subdomProjector(1,:),'YData',subdomProjector(2,:),'EdgeColor','r')
        else
            set(hSubdomPlot,'XData',subDom([1 1 2 2]),'YData',subDom([3 4 4 3]),'EdgeColor','k')

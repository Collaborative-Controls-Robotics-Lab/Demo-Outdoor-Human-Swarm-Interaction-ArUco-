%% Define two transformation, i.e., corners, rvecs & tvecs; and find the translation between the centers
% the transformation from rvecs & tvecs seems to be more stable.
...
hImg = imshow(img);
hold on
hAR_Obstacle1 = patch('XData',nan,'YData',nan,'FaceColor',[1 0 0],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.5);
hold on
hAR_Obstacle2 = patch('XData',nan,'YData',nan,'FaceColor',[1 0 0],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.5);
hold on
hAR_Domain = patch('XData',nan,'YData',nan,'FaceColor','none','EdgeColor','k','LineWidth',4);
hold on
hBlueBox = patch('XData',nan,'YData',nan,'FaceColor',[0.25 0.5 1],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.2);
hold on
hGreenBox = patch('XData',nan,'YData',nan,'FaceColor',[0 1 0.5],'EdgeColor','k','LineWidth',2,'FaceAlpha',0.2);
...
while LoopFlag0
    ...
    if totalIterations0>=2 && length(ids) == numRobots + 1
        ...
        A2 = [[refMarkerCorners2(1,:) 1 0 0 0;
            0 0 0 refMarkerCorners2(1,:) 1;
            refMarkerCorners2(2,:) 1 0 0 0;
            0 0 0 refMarkerCorners2(2,:) 1;
            refMarkerCorners2(3,:) 1 0 0 0;
            0 0 0 refMarkerCorners2(3,:) 1;
            refMarkerCorners2(4,:) 1 0 0 0;
            0 0 0 refMarkerCorners2(4,:) 1],...
            -reshape(repmat(reshape(desiredCoordinates.',[],1),2,1),[],2).*reshape(repmat(reshape(refMarkerCorners2,[],1),1,2).',[],2)];
        b2 = reshape(desiredCoordinates.',[],1);
        % Perspective transformation matrix
        H_perspective = reshape([(A\b);1],3,3)';
        H_inv = inv(H_perspective);
        H_perspective2 = reshape([(A2\b2);1],3,3)';
        H_inv2 = inv(H_perspective2);
        testPts = [-0.5 0.5;0 0];
        resultPts1 =  perspectiveTransform(testPts,H_inv);
        resultPts2 =  perspectiveTransform(testPts,H_inv2);
        scale = norm(resultPts1(:,1)-resultPts1(:,2))/norm(resultPts2(:,1)-resultPts2(:,2));
        transl = sum(resultPts1,2)./2 - sum(resultPts2,2)./2;
        % disp('Perspective transformation matrix & its inverse have been defined')
        % disp('Now implement the augmented reality')
        % disp('-----------------------------------')
        %
        AR_Obstacle1 = invPerspectiveTransform(Obstacle1,H_inv2).*scale + transl;
        AR_Obstacle2 = invPerspectiveTransform(Obstacle2,H_inv2).*scale + transl;
        AR_Domain = invPerspectiveTransform(Obstacle3(:,7:10),H_inv2).*scale + transl;
        AR_Goal1 = invPerspectiveTransform(Goal1,H_inv2).*scale + transl;
        AR_Goal2 = invPerspectiveTransform(Goal2,H_inv2).*scale + transl;

        set(hAR_Obstacle1,'XData',AR_Obstacle1(1,:),'YData',AR_Obstacle1(2,:));
        set(hAR_Obstacle2,'XData',AR_Obstacle2(1,:),'YData',AR_Obstacle2(2,:));
        set(hAR_Domain,'XData',AR_Domain(1,:),'YData',AR_Domain(2,:));
        set(hBlueBox,'XData',AR_Goal1(1,:),'YData',AR_Goal1(2,:));
        set(hGreenBox,'XData',AR_Goal2(1,:),'YData',AR_Goal2(2,:));
    end

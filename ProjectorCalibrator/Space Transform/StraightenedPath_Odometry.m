%% STRAIGHTENEDPATH_ODOMETRY Sets up the space to perform braids on a track.
...
% Number of Braids
M = 80;%2*M_Rectangles+2*numTrap*M_Trapezoids;
...
% Flag to determine whether perfect angular control is used
PerfectAngularControl = 1;

% Angular Velocity Tuning Gain
K_Theta = 20;

% Tolerance around transform to use
...
V = zeros(N,2*M); 
% Allocate vector of switching arc lengths for N Agents, 2 per braid step.
ArcLengths = zeros(N,2*M);
    ...
    for j = 1:N;
        ...
        if(sum(AGENTS==j)==0)
            ...
            if(sum(((1:N)').*(BraidAgents(:,i)==j))==sum(((1:N)').*(BraidAgents(:,i+1)==j)))
                ...
                V(j,[2*i-1 2*i]) = norm((BraidCorners(:,2)-BraidCorners(:,1)))/tf_braid.*[1 1];
                TAU(j,[2*i-1 2*i]) = [tf_braid/2+tf_braid*(i-1), tf_braid*i];
                if(i>1) % We have stored previous arclengths, add to them
                    ArcLengths(j,2*i-1) = norm((BraidCorners(:,2)-BraidCorners(:,1)))/2+ArcLengths(j,2*i-2);
                    ArcLengths(j,2*i) = norm((BraidCorners(:,2)-BraidCorners(:,1)))/2+ArcLengths(j,2*i-1);
                else % ArcLengths is empty don't add the "previous" value 
                    ArcLengths(j,2*i-1) = norm((BraidCorners(:,2)-BraidCorners(:,1)))/2;
                    ArcLengths(j,2*i) = norm((BraidCorners(:,2)-BraidCorners(:,1)))/2+ArcLengths(j,2*i-1);
                end
                AGENTS = [AGENTS,j];
                % keyboard
            ...
            if(size(BraidCorners,2)>2)
                ...
                % Store the switching arclengths for this agent and braid.
                if(i>1) % We have stored previous arclengths, add to them
                    ArcLengths([j NeighborAgent],2*i-1) = Switch_dist+ArcLengths([j NeighborAgent],2*i-2);
                    ArcLengths([j NeighborAgent],2*i) = Total_dist-Switch_dist+ArcLengths([j NeighborAgent],2*i-1);
                else % ArcLengths is empty don't add the "previous" value 
                    ArcLengths([j NeighborAgent],2*i-1) = Switch_dist;
                    ArcLengths([j NeighborAgent],2*i) = Total_dist-Switch_dist+ArcLengths([j NeighborAgent],2*i-1);
                end
...
for i = 1:length(t)-1
    for j = 1:N
        % Determine if we need to switch velocities
%         if(t(i)>TAU(j,Switch_Time_Counter(j)))
        if(Odometer(j)>ArcLengths(j,Switch_Time_Counter(j)))
...
%             keyboard
        ...
        dx_Track = [1,0]*[dTdx1(HH_Braid(:,:,Braid_j,M_BraidCounter),...
                          ...
                          [X(j,i);Y(j,i)]))]*...
                          [dx;dy];
        dy_Track = [0,1]*[dTdx1(HH_Braid(:,:,Braid_j,M_BraidCounter),...
                          T(inv(HH_Braid(:,:,Braid_j,M_BraidCounter)),...
                          [X(j,i);Y(j,i)])),...
                          dTdx2(HH_Braid(:,:,Braid_j,M_BraidCounter),...
                          T(inv(HH_Braid(:,:,Braid_j,M_BraidCounter)),...
        ...
        %     -BraidPoints_Track(:,BraidAgents(:,M_BraidCounter(j))==j,M_BraidCounter(j)));
        % dtheta = K_Theta*[-sin(THETA(j,i)),cos(THETA(j,i))]*...
        %     (BraidPoints_Track(:,BraidAgents(:,(M_BraidCounter+1))==j,M_BraidCounter+1)...
        %     -[X(j,i);Y(j,i)])/...
        ...
        if(PerfectAngularControl==0)
            THETA(j,i+1) =  THETA(j,i) + dtheta*(t(i+1)-t(i));
        else
            THETA(j,i) =  atan2(dy_Track,dx_Track);
        end
        % Get current position in straigthened track
        % Convert the velocities to world plane coordinates
        X(j,i+1) =  X(j,i) + norm([dx_Track;dy_Track])*cos(THETA(j,i))*(t(i+1)-t(i));
        Y(j,i+1) =  Y(j,i) + norm([dx_Track;dy_Track])*sin(THETA(j,i))*(t(i+1)-t(i));
        % X(j,i+1) =  X(j,i) +...
...
%         THETA(j,i+1) =  atan2(HeadingTrack(2),HeadingTrack(1));
        ...
        HeadingTrack(:,j,i+1) = [cos(THETA(j,i+1));sin(THETA(j,i+1))];
...
%                                                               Y(j,i+1)])]...
%                                                              *HeadingTrack(:,j,i+1);   
        Odometer(j) = norm([X_Straightened(j,i+1);
                            Y_Straightened(j,i+1)]...
                          -[X_Straightened(j,i);
                            Y_Straightened(j,i)])...
                      +Odometer(j);
    end
...
% save('Five_Unicycle_20_Braid_Track');
% break
